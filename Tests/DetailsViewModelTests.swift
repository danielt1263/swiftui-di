...
final class DetailsViewModelTests: XCTestCase {
	...
	func testFetchError() {
		...
		let service = MockAPIService(contentFile: "extra-data-sample", isSuccessful: false, jsonDecoder: decoder)
	...
	func testImageDetail() {
		...
		let service = MockAPIService(contentFile: "extra-data-sample", isSuccessful: true, jsonDecoder: decoder)
		let sut = DetailsViewModel(imageModel: image, apiService: service)
		let fetchImagesExpectation = expectation(description: "Fetching image details")
	...
	private func fetchImageDetailList(_ fileName: String) -> ExtraDataResponse? {
		...
		guard let response = try? decoder.decode(ExtraDataResponse.self, from: data) else {
			XCTFail("Could not decode the file.")
			return nil
		}
	...
	private func fetchTestImage() -> ImageModel? {
		...
		guard let image = try? decoder.decode(SampleImagesResponse.self, from: data).sample?.first else {
			return nil
		}
