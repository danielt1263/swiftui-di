...
final class ContentViewModelTests: XCTestCase {
	...
	func testFetch() {
		let service = MockAPIService(contentFile: "images-sample", isSuccessful: true, jsonDecoder: decoder)
	...
	func testFetchError() {
		let service = MockAPIService(contentFile: "images-sample", isSuccessful: false, jsonDecoder: decoder)
		let sut = ContentViewModel(apiService: service)
		let fetchImagesExpectation = expectation(description: "Fetching images")
	...
	func testResponseMissingSample() {
		...
		guard let response = try? decoder.decode(SampleImagesResponse.self, from: data) else {
			XCTFail("Could not read the file.")
			return
		}
	...
	func testBadImageItem() {
		...
		guard let images = try? decoder.decode(SampleImagesResponse.self, from: data).sample else {
